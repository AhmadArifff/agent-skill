...
# Backend Skill â€” Comprehensive Backend Engineer
...
1. ðŸ”’ **Mandatory Admin & App Auth Protection** â€” Jika proyek berkaitan dengan Admin Panel, CRM, Dashboard, atau aplikasi selain landing page publik, WAJIB mengimplementasikan Auth Middleware (JWT/Session) & Authorization (RBAC) yang sangat ketat pada seluruh endpoint API internal untuk mencegah akses tanpa izin dan peretasan.
2. ðŸ› ï¸ **Zero Hardcoded Master Data (Dynamic Master Tables)** â€” Dilarang keras meng-hardcode data master (Label, Priority, Category, Status) di codebase. Buat tabel database dinamis & Endpoint CRUD Admin (`/api/v1/admin/master/*`) agar Admin dapat mengubah data master di runtime tanpa redeploy.
3. ðŸ›¡ï¸ **Pola Guard Clause & Early Return** â€” Hindari *pyramid of doom* (`if-else` bersarang). Lakukan validasi prasyarat di awal fungsi; jika gagal langsung `return Result.fail()` atau lempar *exception*.
4. ðŸ“¦ **Result Pattern pada Service Layer** â€” Kembalikan objek terstruktur `Result.ok(data)` atau `Result.fail(error)` daripada melempar exception acak untuk kesalahan logika bisnis biasa.
5. âš ï¸ **Custom Domain Exception & Centralized Global Exception Handler** â€” Pisahkan error bisnis vs error teknis. Biarkan error teknis *bubble up* ke Global Error Middleware yang mencatat log lengkap dan merespons `500` tanpa membocorkan stack trace ke client.
6. ðŸ“Š **Structured Logging & Observability** â€” Dilarang keras membiarkan blok `catch` kosong. Catat log berstruktur (JSON dengan `userId`, `errorMessage`, `stackTrace`) untuk audittrail dan investigasi bug.
7. ðŸ” **Unambiguous Query Filtering Logic** â€” Logika query database (WHERE clause SQL/Prisma) WAJIB menangani data `NULL`/unassigned secara transparan (misal `WHERE label_id IS NULL`) agar data tidak hilang secara membingungkan saat pengguna memfilter.
8. **Security First** â€” Every endpoint is an attack surface. Validate inputs, authenticate requests, authorize access, and protect data.
9. **Fail Gracefully** â€” Systems will fail. Design for resilience with proper error handling, retries, circuit breakers, and fallbacks.
10. **Document Everything** â€” APIs, database schemas, architecture decisions, and deployment processes must be well-documented.
11. ðŸ“ **5-Step T-C-R-E-I Prompting Framework** â€” Terapkan kerangka kerja 5 langkah (Task, Context, References, Evaluate, Iterate) untuk memformulasi instruksi dan arsitektur backend. Lihat `references/prompting-framework-guide.md`.
...
- **Layered Architecture**: Controller â†’ Service â†’ Repository â†’ Data Source
...
- [ ] No empty `catch` blocks â€” all caught errors are logged with context (`userId`, `stackTrace`)
